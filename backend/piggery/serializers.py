...
from .models import (
    Farm, HealthLog, Pig, WeightRecord, VaccinationRecord,
    DiseaseRecord, BreedingRecord, FeedInventory,
    MedicineInventory, FeedUsageLog, Notification,
)
...
class WeightRecordSerializer(serializers.ModelSerializer):
    class Meta:
        model = WeightRecord
        fields = ["id", "weight_kg", "recorded_at", "notes"]
...
class DiseaseRecordSerializer(serializers.ModelSerializer):
    class Meta:
        model = DiseaseRecord
        fields = ["id", "disease_name", "diagnosed_date", "treatment", "outcome", "resolved_date", "notes"]
...
class BreedingRecordSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "sow", "sow_name", "boar", "breeding_date",
            "expected_farrowing_date", "actual_farrowing_date",
            "pregnancy_status", "piglets_born_alive", "piglets_born_dead", "notes",
        ]
...
class FeedInventorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = FeedInventory
        fields = ["id", "feed_type", "feed_type_display", "stock_kg", "daily_usage_kg", "days_remaining", "last_restocked", "updated_at"]
...
class MedicineInventorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "category", "category_display", "quantity", "unit", "low_stock_threshold", "is_low_stock", "expiry_date", "updated_at"]


class FeedUsageLogSerializer(serializers.ModelSerializer):
    class Meta:
        model = FeedUsageLog
        fields = ["id", "feed", "amount_used_kg", "logged_at", "notes"]


class NotificationSerializer(serializers.ModelSerializer):
    class Meta:
        model = Notification
        fields = ["id", "notification_type", "title", "message", "is_read", "sent_via_sms", "created_at"]
...
class HealthLogSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "severity", "severity_display", "system_findings", "notes",
        ]
        read_only_fields = [
            "pig", "severity", "system_findings",
